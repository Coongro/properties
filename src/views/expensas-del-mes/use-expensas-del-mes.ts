/**
 * Expensas del mes — datos y estado (generado por el Builder de Vistas).
 *
...
export function useExpensasDelMesView() {
  const {
    toast,
    views: { closeDialog },
  } = usePlugin();
  ...
  const validate = useCallback((): Record<string, string> => {
    ...
    if (
      ...
      errs['period'] = '«Período» es requerido';
    if (
      values['amount'] === null ||
      ...
      errs['amount'] = '«Total liquidado» es requerido';
    return errs;
  }, [values]);
  ...
  const submit = useCallback(async () => {
    ...
    try {
      if (customHandlers.onSubmit) {
        ...
        await actions.execute('properties.buildingExpenses.update', {
          id: editingId,
          data: values,
        });
      } else {
        await actions.execute('properties.buildingExpenses.create', { data: values });
      }
