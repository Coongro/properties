/**
 * Propiedad — datos y estado (generado por el Builder de Vistas).
 *
...
export function usePropiedadView() {
  const {
    toast,
    views: { closeDialog },
  } = usePlugin();
  ...
  const validate = useCallback((): Record<string, string> => {
    ...
    if (
      ...
      errs['street'] = '«Calle» es requerido';
    if (
      values['street_number'] === null ||
      values['street_number'] === undefined ||
      values['street_number'] === '' ||
      values['street_number'] === false
    )
      errs['street_number'] = '«Altura» es requerido';
    if (
      values['city'] === null ||
      ...
      errs['city'] = '«Localidad» es requerido';
    return errs;
  }, [values]);
  ...
  const submit = useCallback(async () => {
    ...
    try {
      if (customHandlers.onSubmit) {
        ...
        await actions.execute('properties.buildings.update', { id: editingId, data: values });
      } else {
        await actions.execute('properties.buildings.create', { data: values });
      }
