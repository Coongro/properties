/**
 * Propietarios — datos y estado (generado por el Builder de Vistas).
 *
...
export function usePropietariosView() {
  ...
  const COLUMNS: {
    ...
    { key: 'name', label: 'Propietario' },
    { key: 'document', label: 'Documento', display: 'mono', emptyLabel: 'Sin documento' },
    { key: 'units', label: 'Unidades' },
    { key: 'cbu', label: 'CBU / alias', display: 'mono', emptyLabel: 'Sin datos de cobro' },
  ];
  // el subtítulo se muda bajo el título: fuera de las columnas propias
  const SUB_COL = COLUMNS.find((c) => c.key === 'document');
  const ITEM_COLS = COLUMNS.filter((c) => c.key !== 'document');
