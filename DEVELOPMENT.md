...
- **Trigger name**: `trg_properties_{descriptive_name}`
- **Function name**: `trg_properties_{descriptive_name}_fn`

Where `properties` is your plugin's moduleId (pluginId with hyphens replaced by underscores).
...
CREATE OR REPLACE FUNCTION "trg_properties_audit_log_fn"()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER "trg_properties_audit_log"
AFTER INSERT OR UPDATE ON "module_properties_items"
FOR EACH ROW
EXECUTE FUNCTION "trg_properties_audit_log_fn"();
```

> **Important**: The system automatically **DISABLE**s all `trg_properties_*` triggers when the plugin
